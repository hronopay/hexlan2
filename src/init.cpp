...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Hexlan.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: hexland.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizehexlanamount=<n> " + _("Keep N Hexlan anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Hexlan is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Hexlan is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Hexlan version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Hexlan server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Hexlan") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Hexlan to complete") << "\n";
    ...
    nAnonymizeHexlanAmount = GetArg("-anonymizehexlanamount", 0);
    if(nAnonymizeHexlanAmount > 999999) nAnonymizeHexlanAmount = 999999;
    if(nAnonymizeHexlanAmount < 2) nAnonymizeHexlanAmount = 2;
    ...
    LogPrintf("Anonymize Hexlan Amount %d\n", nAnonymizeHexlanAmount);
