...
const string strMessageMagic = "Hexlan Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 16 * COIN; // instamine prevention
    }
    else if (nHeight > 400 && nHeight <= 1499) {
        nSubsidy = 24 * COIN; // initial block reward
    }
    else if (nHeight > 1499 && nHeight <= 195000) {
        nSubsidy = 12 * COIN; // initial block reward
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (nBestHeight+1 > 46050 && nBestHeight+1 <= 390000)  {
        nSubsidy = 128 * COIN;
    }
    else if (nBestHeight+1 > 390000 && nBestHeight+1 <= 585000) {
        nSubsidy = 8 * COIN;
    }
    else if (nBestHeight+1 > 585000 && nBestHeight+1 <= 780000) {
        nSubsidy = 4 * COIN;
    }
    else if (nBestHeight+1 > 780000 && nBestHeight+1 <= 1950000) {
	// end game - further discussion needed
        nSubsidy = 3 * COIN;
    } else if (nBestHeight+1 > 1950000) {
	// end game - further discussion needed
        nSubsidy = 2 * COIN;
        nSubsidy >>= ((nBestHeight + 195000) / 97500);
    }
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    for(int k=0; k<supposedMnList.sizeMn(); k++){
                        if(rewPayee == supposedMnList.getValueMn(k)) foundInList = true;
                        LogPrintf("CheckBlock() : k= %d , supposedMnList.getValueMn(k)= %s \n", k, supposedMnList.getValueMn(k));
                    }
